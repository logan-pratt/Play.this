...
@IBDesignable class SpringySlider: UIControl
{
    ...
    @IBInspectable internal var trackTintColor: UIColor = UIColor.whiteColor()  {
        didSet {
            ...
            self.maxTrackLayer.backgroundColor = UIColor(red:0.66, green:0.66, blue:0.66, alpha:1.0).CGColor
        }
    }
    
    @IBInspectable internal var thumbImage: UIImage? {
        didSet {
            var widthSize = thumbImage!.size.width / 10
            var heightSize = thumbImage!.size.height / 10
            self.thumbLayer.contents = thumbImage!.CGImage
            self.thumbLayer.frame.size = CGSize(width: widthSize, height: heightSize)
    ...
    private func setup() -> ()
    {
        ...
        self.thumbLayer.frame = CGRectMake(0, 0, 42.5, 51)
